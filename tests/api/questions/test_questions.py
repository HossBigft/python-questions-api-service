...
async def test_get_questions_with_pagination(
    ...
    r = await client.get("/questions/?skip=0&limit=10", headers=superuser_token_headers)
    assert r.status_code == 200
    data = r.json()
    assert len(data) == 10
...
async def test_delete_question_success(
    ...
    r = await client.delete(f"/questions/{q.id}", headers=superuser_token_headers)
    assert r.status_code == 200
...
async def test_delete_question_not_found(
    ...
    r = await client.delete("/questions/999999", headers=superuser_token_headers)
...
async def test_add_answer_to_question_success(
    ...
    r = await client.post(
        f"/questions/{q.id}/answers", json=answer_data, headers=superuser_token_headers
    )
    assert r.status_code == 200
...
async def test_add_answer_to_nonexistent_question(
    ...
    r = await client.post(
        "/questions/999999/answers", json=answer_data, headers=superuser_token_headers
    )
    assert r.status_code == 404
